...
if (this.linearRotTrans.length > 16) this.setGamma ();
...
var pt = 0;
for (var i = 0; i < 3; i++) {
for (var j = 0; j < 3; j++) a[i][j] = this.linearRotTrans[pt++];

a[i][n] = this.linearRotTrans[pt++];
}
pt += 4;
for (var i = 3; i < 3 + this.modDim; i++) {
for (var j = 3; j < 3 + this.modDim; j++) a[i][j] = this.linearRotTrans[pt++];
...
this.modDim = modDim;
if (modDim > 0) {
n = 16 + (modDim + 1) * (modDim + 1);
this.myLabels = J.symmetry.SymmetryOperation.labelsXn;
...
return true;
}var strOut = J.symmetry.SymmetryOperation.getMatrixFromString (this, xyz, this.linearRotTrans, false);
...
linearRotTrans[15] = 1;
if (modDim > 0) linearRotTrans[linearRotTrans.length - 1] = 1;
var myLabels = (op == null || modDim == 0 ? null : op.myLabels);
if (myLabels == null) myLabels = J.symmetry.SymmetryOperation.labelsXYZ;
xyz = xyz.toLowerCase ();
xyz += ",";
if (modDim > 0) for (var i = J.symmetry.SymmetryOperation.labelsXn.length; --i >= 0; ) xyz = JU.PT.simpleReplace (xyz, J.symmetry.SymmetryOperation.labelsXn[i], J.symmetry.SymmetryOperation.labelsXnSub[i]);
...
var ipt = ch.charCodeAt (0) - (ch >= 'x' ? 'x' : 'a').charCodeAt (0);
linearRotTrans[tpt0 + ipt] = val;
strT += J.symmetry.SymmetryOperation.plusMinus (strT, val, myLabels[ipt + (rowPt < 3 ? 0 : 3)]);
break;
case ',':
...
$_M(c$, "toString", 
function () {
return (this.rsvs == null ? Clazz.superCall (this, J.symmetry.SymmetryOperation, "toString", []) : this.rsvs.toString ());
});
