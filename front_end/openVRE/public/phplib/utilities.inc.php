...
function getUtilitiesLogger()
{
    ...
    if ($logger === null) {
        $logger = LoggerFactory::getLogger('Utilities interface');
    }
...
function saveMetadataUpload($fn, $request, $validationState)
{
    ...
    // save to mongo
    modifyMetadataBNS($fn, $insertMeta);
}
...
function prepMetadataUpload($request, $validationState = 0)
{
    ...
    if (isset($request['refGenome'])) {
        $insertMeta['refGenome']  = $request['refGenome'];
    }
    // BAM metadata
    if (isset($request['paired'])) {
        $insertMeta['paired']     = $request['paired'];
    }
    if (isset($request['sorted'])) {
        $insertMeta['sorted']     = $request['sorted'];
    }
    if (isset($request['description'])) {
        $insertMeta['description'] = $request['description'];
    }
    //  results metadata
    if (isset($request['submission_file'])) {
        $insertMeta['submission_file'] = $request['shFile'];
    }
    if (isset($request['log_file'])) {
        $insertMeta['log_file'] = $request['logFile'];
    }
...
function prepMetadataResult($meta, $fnPath = 0, $lastjob = array())
{
    ...
    if (is_null($meta['input_files']) && isset($lastjob['input_files'])) {
        $input_ids = array();
        array_walk_recursive($lastjob['input_files'], function ($v, $k) use (&$input_ids) {
            $input_ids[] = $v;
        });
    ...
    if (is_null($meta['refGenome']) && in_array($meta['format'], array("BAM", "GFF", "GFF3", "BW"))) {
        ...
        if (is_null($meta['refGenome']) && $fnPath) {
            $fnCore   = "";
            $ext = $meta['format'];
...
function prepMetadataLog($metaOutfile)
{
    ...
    if (is_null($metaLog['data_type'])) {
        $metaLog['data_type'] = "data_log";
    }
...
function validateOpenSSHPrivateKey($key)
{
    ...
    if (
        ...
        getUtilitiesLogger()->error("Invalid OpenSSH private key headers.");
    ...
    if ($decodedKey === false) {
        getUtilitiesLogger()->error("Base64 decoding failed. The key body might be corrupted.");
    ...
    if (substr($decodedKey, 0, 15) !== "openssh-key-v1\0") {
        getUtilitiesLogger()->error("Invalid OpenSSH key format.");
        
        return false;
    }

    getUtilitiesLogger()->info("The key is a valid OpenSSH private key.");
