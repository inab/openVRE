...
function prepare_getData_fromURL($url, $outdir, $referer, $meta = []) {
    ...
    if ($workingDirId == "0") {
        ...
        $IsMetadataAdded = addMetadataBNS($workingDirId, ["expiration" => -1,
    ...
    $toolArgs  = [
            ...
            "output" => $filePath];           // Tool is responsible to create outputs in the output_dir
...
function getData_fromRepository($url, $datatype, $filetype, $description, $oeb_dataset_id, $oeb_community_ids) {   
    ...
    if ($workingDirId == "0") {
        ...
        $addedMetadata = addMetadataBNS($workingDirId, ["expiration" => -1,
                    "description"=> "Remote personal data"]);
        if ($addedMetadata == "0") {
