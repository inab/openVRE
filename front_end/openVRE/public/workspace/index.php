			...
			<div class="page-content">
				...
				<h1 class="page-title"> <?php echo getProject($_SESSION['User']['dataDir'])["name"]; ?> Workspace
					...
					<div class="input-group" style="float:right; width:200px; margin-right:10px;">
						<span class="input-group-addon" style="background:#5e738b;"><i
														...
														<div class="input-group">
															...
															<span class="input-group-btn">
																<button class="btn green mt-clipboard"
																	...
																	data-clipboard-target="#mt-target-1" type="button"><i
																		class="fa fa-copy"></i> Copy to clipboard</button>
															</span>
														</div>
														<br />
								...
								<div class="portlet light bordered">
									...
									<div class="portlet-title">
										<div class="caption">
											...
											<span class="caption-subject font-dark bold uppercase">Select File(s)</span>
											<small style="font-size:75%;">Please select the file or files you want to
												use</small>
										</div>
										<div class="actions">
											<a href="<?php echo $GLOBALS['BASEURL']; ?>workspace/" class="btn green">
												Reload Workspace </a>
										</div>
									</div>
									...
									<div class="portlet-body">
										...
										<div class="input-group" style="margin-bottom:20px;">
											<span class="input-group-addon" style="background:#5e738b;"><i
													class="fa fa-wrench font-white"></i></span>
											<select class="form-control" style="width:100%;"
												onchange="loadWSTool(this)">
										...
										<form name="gesdir" action="workspace/workspace.php" method="post"
						...
						<div class="portlet light bordered">
								...
								<div class="actions" style="display:none!important;" id="btn-av-tools">
									<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right" role="menu">
											<li><a href="javascript:downloadAllFiles();"><i class="fa fa-download"></i>
													Download selected files </a></li>
											<li><a href="javascript:editAllFiles();"><i class="fa fa-pencil"></i>
													Edit
													selected files metadata </a></li>
											<li><a href="javascript:deleteAllFiles();"><i class="fa fa-trash-o"></i>
													Delete selected files </a></li>
											<li><a href="javascript:moveAllFiles();"><i class="fa fa-exchange"></i>
													Move
													selected files </a></li>
										</ul>
									</div>
									<div class="btn-group">
										<a class="btn btn-sm purple-intense" id="visualization" href="javascript:;"
											data-toggle="dropdown">
											<i class="fa fa-eye"></i> Visualization
											<i class="fa fa-angle-down"></i>
										</a>
										<ul class="dropdown-menu pull-right" id="visualizers_list" role="menu">
										</ul>
									</div>
									<div class="btn-group">
										<a class="btn btn-sm blue-dark" id="av_tools" href="javascript:;"
											data-toggle="dropdown">
							...
							<div class="portlet-body">
								<div class="" data-always-visible="1" data-rail-visible="0">
									...
									<div id="desc-run-tools">In order to run the tools on the files, please select
										them
										clicking on the checkboxes from the table above.</div>
								</div>
								<div class="scroller-footer">
									<a class="btn btn-sm red pull-right display-hide" id="btn-rmv-all"
										href="javascript:;">
													...
													<div class="panel-heading">
														<h4 class="panel-title">
															<a class="accordion-toggle accordion-toggle-styled "
																...
																href="#collapse_<?php echo $c; ?>"> <?php echo $tl["name"]; ?>
															</a>
														</h4>
													</div>
																		...
																		if ($th["id"] == $tl["_id"]) { ?>
																			...
																			foreach ($th["content"] as $content) { ?>
																				...
																				<tr class="<?php echo $trclass; ?>">
																					<?php if ($cc == 1) { ?>
																						<td rowspan="<?php echo sizeof($th["content"]); ?>">
																							<?php echo $th["operation"]; ?>
																						</td>
																					<?php } ?>
																					<td><?php echo $content["description"]; ?></td>
																					<td><?php echo implode("<br>", $content["format"]); ?>
																					</td>
					...
					<div class="col-lg-6 col-xs-12 col-sm-12">
						<div class="portlet light tasks-widget bordered">
							...
							<div class="portlet-body">
								...
								<div id="disk-home">
									<input class="knob" data-fgColor="#006b8f" data-bgColor="#eeeeee" readonly
										value="<?php echo $usedDiskPerc; ?>" />

									<div id="info-disk-home">
										You are using <strong><?php echo formatSize($usedDisk); ?></strong> from
										your
										<strong><?php echo formatSize($diskLimit); ?></strong> of disk quota.
									</div>
									...
									<div id="extra-space-home">
										<?php if (allowedRoles($_SESSION['User']['Type'], $GLOBALS['NO_GUEST'])) { ?>
											...
											<a href="<?php echo $GLOBALS['BASEURL']; ?>helpdesk/?sel=space"
												class="btn green">I need more disk space</a>
										<?php } else { ?>
											Do you want extra disk space? Click the button below to register!
											<br><br>
											<a href="<?php echo $GLOBALS['URL_login']; ?>" class="btn green">Register to
												VRE</a>
										<?php } ?>
									</div>
								</div>
		...
		<div class="modal fade bs-modal-sm" id="myModal1" tabindex="-1" role="basic" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body"> You have more than one file selected. If you go ahead, this tool will
						just
						be applied to the selected file. </div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div id="loading-viewport" style="position:absolute;left:42%; top:200px;"><img
								src="assets/layouts/layout/img/ring-alt.gif" /></div>
...
If you want to <strong>re-use your session</strong>, make sure you save the <strong><em>'Restore link'</em></strong> that appears on your workspace. Otherwise, your data will be unreachable in the moment the session ends. <br/>
														<img source=""/>-->
						As an <strong>anonymous user</strong>, you can access to all VRE functionalities, yet your
						workspace is not permanent!
					</div>
					<div class="modal-footer">
						...
						<div id="form-rename" class="display-hide">
							<form action="">
								<div class="form-group" id="">
									...
									<div class="input-group">
										<input type="text" class="form-control " name="" id="new-name"
											placeholder="Please enter the file name" value="">
										<div class="input-group-btn">
											<button type="button" class="btn green" data-toggle="dropdown"
												id="submit-rename">Submit <i class="fa fa-check"></i></button>
										</div>
									</div>
								</div>
							</form>
						...
						<div class="row display-hide" id="move-file">
							...
							<div class="col-md-3" id="col-3-move">
								<div class="form-group">
									...
									<input type="text" class="form-control " name="" id="new-name-move-file"
										placeholder="Please enter the file name" value="">
								</div>
							</div>
							<div class="col-md-3" id="col-4-move">
								<div class="form-group">
									...
									<button type="button" class="btn green form-control" data-toggle="dropdown"
										id="submit-move">Submit <i class="fa fa-check"></i></button>
								</div>
							</div>
						</div>
						<div class="alert alert-danger display-hide"> </div>
				...
				<div class="modal-content">
					...
					<div class="modal-body table-responsive">
						<div style="max-height: calc(100vh - 255px);padding-left:20px;">
							...
							<div id="meta-log" class="display-hide" style="padding: 0 0 20px 0; color:#949494">
							</div>
						</div>
					</div>
					<div class="modal-footer" style="text-align:left;">
						...
						<button type="button" class="btn dark btn-outline" data-dismiss="modal"
							style="right: 15px;position: absolute;">Close</button>
					</div>
				</div>
			</div>
		</div>
